...
let { config: _config, storage_name: _storage_name, project_name } = require('@/simpleConfig.js')
let Timers = require('@/lib/prototype/Timers')
let _runningQueueDispatcher = require('@/lib/prototype/RunningQueueDispatcher')
let _FloatyInstance = require('@/lib/prototype/FloatyUtil')
let automator = require('@/lib/prototype/Automator')
let FileUtils = require('@/lib/prototype/FileUtils')
let _logUtils = require('@/lib/prototype/LogUtils')
let processShare = require('@/lib/prototype/ProcessShare')
let capturePermissionResolver = require('@/lib/prototype/CapturePermissionResolver')
let formatDate = require('@/lib/DateUtil.js')
let { updateChecker } = require('@/lib/UpdateChecker.js')
...
function CommonFunctions () {
  ...
  this.clickBackOrClose = function (packageName) {
    ...
    if (!this._widgetUtils) {
      this._widgetUtils = require('@/lib/prototype/WidgetUtils')
    }
  ...
  */
  ...
  this.requestScreenCaptureOrRestart = function (doNotRestart) {
    ...
    if (_config.request_capture_permission && this.ensureAccessibilityEnabled()) {
      // 存在循环依赖，待解决
      screenPermission = require('@/lib/prototype/RequestScreenCapture')()
    } else {
  ...
  this.convertImageFromSingleChannel = function (singleChannelImage) {
    ...
    if (typeof resourceMonitor === 'undefined') {
      // 避免循环引用对象
      require('@/lib/ResourceMonitor.js')(runtime, _global_this_).addImageToList(img)
    } else {
